              ...
              <div className="grid grid-cols-7 items-center mb-4">
                <div>
                  {item.images && item.images.length > 0 && item.images[0] ? (
                    <img
                      src={`${import.meta.env.VITE_API_URL || window.location.origin}/uploads/${item.images[0]}`}
                      alt={item.name}
                      className="w-20 h-20 object-cover rounded"
                    />
                  ) : (
                    <div className="w-20 h-20 flex items-center justify-center bg-gray-200 text-gray-400 rounded border border-gray-300">
                      No Image
                    </div>
                  )}
                </div>
