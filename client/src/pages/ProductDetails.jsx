...
const ProductDetails = () => {
  const { productsData, currency, addToCart, addToFavorite, axios } =
    useContext(AppContext);

  const { id } = useParams();
  const [product, setProduct] = useState(null);
  useEffect(() => {
    const foundProduct = productsData.find((item) => String(item._id) === String(id));
    setProduct(foundProduct);
    if (foundProduct?.images?.length > 0) {
      // Resolve to a full URL if backend returns a filename
      const base = axios?.defaults?.baseURL ?? import.meta.env.VITE_BASEURL ?? "";
      const first = foundProduct.images[0];
      let resolved = "";
      if (typeof first === "string") {
        if (first.startsWith("http") || first.startsWith("//") || first.startsWith("/")) resolved = first;
        else resolved = `${base.replace(/\/$/,"")}/uploads/${first}`;
      } else if (typeof first === "object") {
        const url = first.url ?? first.path ?? first.filename ?? "";
        if (url.startsWith("http") || url.startsWith("//") || url.startsWith("/")) resolved = url;
        else resolved = `${base.replace(/\/$/,"")}/uploads/${url}`;
      }
      setMainImage(resolved || "https://via.placeholder.com/500x400?text=No+Image");
    }
  }, [id, productsData, axios]);
          ...
          <div className="grid grid-cols-4 gap-4 w-full max-w-2xl">
            {product?.images?.map((img, index) => {
              ...
              if (typeof img === "string") {
                if (img.startsWith("http") || img.startsWith("//") || img.startsWith("/")) src = img;
                else src = `${base.replace(/\/$/,"")}/uploads/${img}`;
              } else if (typeof img === "object") {
                const url = img.url ?? img.path ?? img.filename ?? "";
                if (url.startsWith("http") || url.startsWith("//") || url.startsWith("/")) src = url;
                else src = `${base.replace(/\/$/,"")}/uploads/${url}`;
              }
              return (
                <img
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-start">
          ...
          <div className="flex items-center space-x-4 mt-4">
            ...
            <h2 className="text-lg font-bold text-gray-800">
              {currency}{product?.offerPrice?.toLocaleString('en-IN')}
            </h2>
          </div>
